...
def simulate_strategy(
    ...
    *,
    ...
    En cada fecha de aportación se decide el importe (según la estrategia y la señal de
    oportunidad vigente), se compran `importe / nav` unidades y se acumulan. Al final se
    valora la posición al último NAV disponible.
    ...
    for date in dates:
        ...
        if price is None or pd.isna(price) or price <= 0:
            continue
        sig = signal.get(date, float("nan"))
        amount = contribution_amount(strategy, sig, base, cfg)
        if amount <= 0:
            continue
        units += amount / price
        invested += amount
